...
重装前让当前系统里的 Codex/AI 做这些事：

1. 用 `codex-rehome` 打包当前 Codex 数据。
2. 把要保留的项目文件夹一起打包，尤其是放在桌面、下载、文档、C 盘项目目录里的文件。
3. 把输出目录指定到非系统盘或移动硬盘，例如 `D:\Codex-Rehome-Backup`、`E:\Codex-Rehome-Backup`、`F:\Codex-Rehome-Backup`。
4. 在同一个备份目录里保存 GitHub 地址、README 截图，或者一段给新系统 Codex 的恢复说明文字。
5. 确认 zip、manifest、校验文件和恢复说明都在非 C 盘后，再重装系统。
...
重装后，先安装并登录 Codex，然后把备份目录里的 zip 和恢复说明交给新系统里的 Codex/AI，让它解压、恢复、注册项目并运行 verifier。
...
先把预期说清楚：`codex-rehome` 不是魔法搬家，也不是官方云同步。它能尽量恢复 Codex 的本地协作现场，但有些东西不能保证 100% 原样复活。

- **不能保证旧聊天框继续操控原项目目录。** 历史对话和 session 可以恢复，但旧对话里记录的工作目录可能是 `C:\Users\...\project`，到了 Mac 上会变成 `~/Documents/Codex-Restored-Projects/project`。路径模型不一样，所以旧聊天框里的“工作目录句柄”不一定还能继续执行文件操作。
- **更推荐的续接方式是：旧对话当历史上下文，新项目重新打开。** 迁移后先看旧对话找回思路和上下文，再从恢复后的项目文件夹重新打开一个 Codex 项目对话继续工作。
- **项目文件不会凭空出现。** Codex 的对话数据和你的项目源代码是两件事。旧电脑打包时必须用 `--project` 或 `-Project` 把需要的项目文件夹一起放进包里。
- **左侧栏显示不是单纯复制文件能保证的。** 恢复脚本会尝试调用 `codex app <恢复后的项目路径>` 注册项目。如果被系统权限或 Codex Desktop 状态挡住，就需要用户在 Codex Desktop 里手动打开恢复后的项目文件夹。
- **登录态默认不迁移。** Codex、GitHub、飞书、Gmail、浏览器扩展、MCP/connectors 等可能需要在新电脑重新登录。
- **敏感文件默认不迁移。** auth token、浏览器 cookies、Login Data、Local Storage、`.env`、API key、私钥、`.git`、`node_modules`、虚拟环境等默认排除。
- **运行中的状态不能迁移。** 打开的终端、未保存的编辑器内容、正在运行的进程、桌面窗口布局、内存里的临时状态，都不属于迁移范围。
- **依赖环境需要重装。** `node_modules`、Python venv、编译产物、系统原生依赖、游戏/工具运行时等，到了目标电脑一般要重新安装或重新构建。
